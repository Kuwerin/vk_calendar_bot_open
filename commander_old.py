...
#            self.start_time = self.procedure_date.isoformat()
            self.duration = timedelta(minutes=self.duration)
            self.end_time = self.procedure_date + self.duration
#            self.start_time = self.procedure_date.isoformat()
#            self.start_time.isoformat()  # попробовать! Вроде бы это как раз тот формат, что нам нужен!
            print(self.start_time)
            print(self.end_time)
            response = ""
            if check_time(self.procedure_date, self.end_time) == "time":  # можно поменять на default и все заработает, но это заебет людей
                response = (self.username + ", вы выбрали занятое время, попробуйте еще раз!")
            elif check_time(self.procedure_date, self.end_time) == "confirmation":
...
#        if response == "time":
    ...
    def clear_input(self):
        """
        Обновить!!! ВСЕ СЕЛФЫ из инита загнать сюда кроме expire time!!
        :return:
        """
        # Текущий, предыдущий режимы
        self.username = ""
    ...
    def reload(self):
        """
        Служебная херня. Релоадит текущего пользователя
        """
        # Текущий, предыдущий режимы
        self.username = ""
        self.full_name = ""
